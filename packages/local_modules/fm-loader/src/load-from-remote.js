...
export const loadVarFromRemote = ({ remote = {}, varible } = {}) => {
  const { name, url } = remote;

  if (!url) throw new Error("Missing remote url");
  if (!name) throw new Error("Missing remote name");
  if (!varible) throw new Error("Missing varible");
  ...
  return async () => {
    ...
    const factory = await container.get(varible);

    if (!factory)
      throw new Error(
        `Cannot load ${varible} in remote: ${name} from url ${url}`
      );
